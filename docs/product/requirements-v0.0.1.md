# TeamClaw v0.0.1（OpenClaw Plugin 全栈方案）需求文档

> 范围说明：
> - v0.0.1 仍是 OpenClaw-first。
> - "多 Agent"在此定义为：同一个 OpenClaw 连接下可选择不同独立 agent（各自拥有独立 workspace、IDENTITY.md、SOUL.md、memory）执行。
> - 不涉及新增外部 provider（如 Claude 直连），仅限 OpenClaw 体系内多 agent。多 provider 支持依赖未来 OpenClaw 生态更新。
...
TeamClaw 作为一个 **OpenClaw Channel Plugin**（`@teamclaw/openclaw-plugin`）运行在 Gateway 进程内部。插件同时承载前端 SPA、后端 API、数据持久化和 Agent 交互，用户只需安装插件即可使用。

### 代码分层（Ports & Adapters）

插件内部采用 **端口与适配器** 架构，将 OpenClaw SDK 依赖隔离在适配器层，核心业务逻辑不依赖任何 OpenClaw 包：
...
│
├── src/adapter/                         ← OpenClaw 适配器（可替换）
│     └── openclaw-provider.ts              实现 AgentProvider 接口
│           ├── agents.list RPC             → listAgents()
│           ├── dispatchInboundReplyWithBase → dispatch()
│           └── deliver() 回调               → onResult callback
│
├── src/core/                            ← 纯业务逻辑（不 import openclaw/*）
...
```
index.ts → adapter/ → openclaw/plugin-sdk
               ↓
...
- `core/` 只依赖 `ports.ts` 中定义的接口，不依赖 `adapter/` 或 `openclaw/*`。
- `adapter/` 是唯一 import OpenClaw SDK 的地方。
- 替换 adapter 即可对接其他 Agent provider，core 层零改动。

### 核心接口（ports.ts）

```typescript
// core/ports.ts — 纯 TypeScript，不依赖任何 OpenClaw 包
...
| Agent 交互 | Plugin SDK Inbound Dispatch | 隔离在 `adapter/` 层，core 通过 `AgentProvider` 接口调用 |
...
> 用户只需安装插件并打开浏览器。无需配置双侧连接、无需 shared_secret。

### A1. 安装

- **AC-SETUP-001**：用户通过 `openclaw plugins install @teamclaw/openclaw-plugin` 安装插件。
...
> 插件在同一进程内直接调用 Gateway `agents.list` RPC，无需 HTTP 代理。

### B1. 获取 Agent 列表

- **AC-AGENT-001**：前端通过 `GET /plugins/teamclaw/api/agents` 获取可用 agent 列表。
- **AC-AGENT-002**：API 层调用 `AgentProvider.listAgents()`（adapter 内部调用 Gateway `agents.list` RPC），返回字段包含：
  - `agent_id`（string，标识）
  - `name`（string，显示名）
...
> Run 的全部生命周期在插件进程内完成：前端发起 → API 层校验 → Plugin SDK dispatch → agent 回复 → 写入数据库 → 前端轮询/推送获取结果。
...
- **AC-RUN-004**：API 层异步调用 `AgentProvider.dispatch()`，由 adapter 层负责构造 OpenClaw route 并调用 SDK：
  ```typescript
...
- **AC-SET-001**：设置页包含两个区块：
  1) Agent Catalog（可用独立 agent 列表，实时从 Gateway 获取）
...
- **AC-SEC-001**：Web UI 的访问控制复用 OpenClaw Gateway 的认证机制（Gateway 已有 connection-auth）。
- **AC-SEC-002**：若 Gateway 未启用认证（本地开发场景），插件允许匿名访问。
...
- **AC-FB-002**：Agent 列表 RPC 失败时返回缓存列表（若存在）。
...
> 插件使用 SQLite 持久化所有业务数据，存储于 OpenClaw 状态目录。
...
| 变更 | 说明 |
|------|------|
| 架构重构（B 方案） | 从"插件 + 独立 Web 服务"改为"全栈插件"——SPA/API/数据库全部运行在插件内 |
| 代码分层（Ports & Adapters） | 引入 `AgentProvider` 接口隔离 OpenClaw SDK 依赖；core 层纯业务逻辑不依赖任何 OpenClaw 包 |
| 技术栈变更 | Next.js → Vite + React SPA；独立部署 → `openclaw plugins install` 一步完成 |
| 删除跨服务通信 | 删除所有 HTTP callback、HMAC 签名、shared_secret 相关 AC——不再需要跨服务通信 |
| 删除 Section A 双侧配置 | 改为极简的安装配置（AC-SETUP-001~005） |
| 删除 Section F 通信安全 | 无跨服务通信，复用 Gateway 认证机制 |
| 新增 Section I 数据模型 | SQLite 表结构定义 |
| 新增 Section J API 路由汇总 | 完整的路由列表（静态资源 + REST API） |
| 简化 Section H | 无跨服务场景，降级策略大幅简化 |
| AC 编号全局重排 | 删除 `PLG-` 前缀——不再区分"插件侧"和"Web 侧"，因为它们是同一个应用 |
